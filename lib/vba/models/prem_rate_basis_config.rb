  ...
  class PremRateBasisConfig
    ...
    def self.attribute_map
      {
        ...
        :'prem_rate_basis_description' => :'premRate_Basis_Description',
        :'rates' => :'rates'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::PremRateBasisConfig` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::PremRateBasisConfig`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [prem_rate_basis, prem_rate_basis_description, rates].hash
    end
