  ...
  class ProviderSuffix
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::ProviderSuffix` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::ProviderSuffix`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @suffix.nil?
        invalid_properties.push('invalid value for "suffix", suffix cannot be nil.')
      end

      if @suffix.to_s.length > 16
        invalid_properties.push('invalid value for "suffix", the character length must be smaller than or equal to 16.')
      end

      if @suffix.to_s.length < 1
    ...
    def valid?
      ...
      return false if @suffix.to_s.length > 16
      return false if @suffix.to_s.length < 1
    ...
    def suffix=(suffix)
      if suffix.nil?
        fail ArgumentError, 'suffix cannot be nil'
      end

      if suffix.to_s.length > 16
        fail ArgumentError, 'invalid value for "suffix", the character length must be smaller than or equal to 16.'
      end
    ...
    def hash
      [provider_id, suffix, entry_date, entry_user, update_date, update_user].hash
    end
