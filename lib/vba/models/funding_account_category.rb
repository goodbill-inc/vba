...
module Vba
  class FundingAccountCategory
    # Name of the 'Funding Account Category'.
    attr_accessor :account_category

    # Description of the 'Funding Account Category'.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::FundingAccountCategory` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::FundingAccountCategory`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @account_category.to_s.length > 8
        invalid_properties.push('invalid value for "account_category", the character length must be smaller than or equal to 8.')
      end

      if @account_category.to_s.length < 1
        invalid_properties.push('invalid value for "account_category", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @account_category.to_s.length > 8
      return false if @account_category.to_s.length < 1
    ...
    def account_category=(account_category)
      if account_category.nil?
        fail ArgumentError, 'account_category cannot be nil'
      end
      ...
      if account_category.to_s.length < 1
        fail ArgumentError, 'invalid value for "account_category", the character length must be great than or equal to 1.'
      end
    ...
    def hash
      [account_category, description, entry_date, entry_user, update_date, update_user].hash
    end
