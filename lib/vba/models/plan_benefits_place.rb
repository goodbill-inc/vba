...
module Vba
  class PlanBenefitsPlace
    ...
    # This allows the User to apply the speecified Ex Code to any claims containing the selected place code.
    ...
    # This is the Ex Code that will be applied to any claims containing the selected place code.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::PlanBenefitsPlace` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::PlanBenefitsPlace`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [plan_id, benefit_code, place_code, entry_date, entry_user, ex_code, update_date, update_user].hash
    end
