  ...
  class CLIACertificate
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::CLIACertificate` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::CLIACertificate`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@certificate_type.nil? && @certificate_type.to_s.length > 32
        invalid_properties.push('invalid value for "certificate_type", the character length must be smaller than or equal to 32.')
      end
    ...
    def valid?
      ...
      return false if !@certificate_type.nil? && @certificate_type.to_s.length > 32
    ...
    # Custom attribute writer method with validation
    # @param [Object] certificate_type Value to be assigned
    def certificate_type=(certificate_type)
      if !certificate_type.nil? && certificate_type.to_s.length > 32
        fail ArgumentError, 'invalid value for "certificate_type", the character length must be smaller than or equal to 32.'
      end
    ...
    def hash
      [certificate_id, certificate_type, description, entry_date, entry_user, update_date, update_user].hash
    end
