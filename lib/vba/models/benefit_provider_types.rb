...
module Vba
  class BenefitProviderTypes
    # References the BenefitCodes.Benefit_Code from the parent table.
    attr_accessor :benefit_code

    # Specifies the Provider type that the User wishes to either include or exclude as part of the benefit.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::BenefitProviderTypes` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::BenefitProviderTypes`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [benefit_code, provider_type, entry_date, entry_user, exclude, update_date, update_user].hash
    end
