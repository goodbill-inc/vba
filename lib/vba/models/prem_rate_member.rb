  ...
  class PremRateMember
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::PremRateMember` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::PremRateMember`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'rate_override')
        self.rate_override = attributes[:'rate_override']
      end
    ...
    def hash
      [prem_rate_key, subscriber_id, member_seq, effective_date, entry_date, entry_user, rate_override, term_date, update_date, update_user].hash
    end
