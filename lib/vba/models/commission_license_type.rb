  ...
  class CommissionLicenseType
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::CommissionLicenseType` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::CommissionLicenseType`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @license_type.to_s.length > 32
        invalid_properties.push('invalid value for "license_type", the character length must be smaller than or equal to 32.')
      end

      if @license_type.to_s.length < 1
        invalid_properties.push('invalid value for "license_type", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @license_type.to_s.length > 32
      return false if @license_type.to_s.length < 1
    ...
    def license_type=(license_type)
      ...
      if license_type.to_s.length < 1
        fail ArgumentError, 'invalid value for "license_type", the character length must be great than or equal to 1.'
      end
    ...
    def hash
      [license_type, description, entry_date, entry_user, update_date, update_user].hash
    end
