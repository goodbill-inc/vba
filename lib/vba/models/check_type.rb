...
module Vba
  class CheckType
    # Name for the check type.
    attr_accessor :check_type

    # Description of the check status.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::CheckType` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::CheckType`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @check_type.to_s.length > 3
        invalid_properties.push('invalid value for "check_type", the character length must be smaller than or equal to 3.')
      end

      if @check_type.to_s.length < 1
        invalid_properties.push('invalid value for "check_type", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @check_type.to_s.length > 3
      return false if @check_type.to_s.length < 1
    ...
    def check_type=(check_type)
      ...
      if check_type.to_s.length < 1
        fail ArgumentError, 'invalid value for "check_type", the character length must be great than or equal to 1.'
      end
    ...
    def hash
      [check_type, description, entry_date, entry_user, update_date, update_user].hash
    end
