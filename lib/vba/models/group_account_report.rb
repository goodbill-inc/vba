...
module Vba
  class GroupAccountReport
    # References Groups.Group_ID value on parent record.
    attr_accessor :group_id

    # References PayorAccount.Account_Key value on parent record. Note: This field is marked as NOT NULL in the database and therefore required by the API.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::GroupAccountReport` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::GroupAccountReport`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [group_id, account_key, entry_date, entry_user, payor_id, update_date, update_user].hash
    end
