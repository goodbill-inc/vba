  ...
  class GroupDivisionIDCode
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::GroupDivisionIDCode` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::GroupDivisionIDCode`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @i_d_code.to_s.length > 6
        invalid_properties.push('invalid value for "i_d_code", the character length must be smaller than or equal to 6.')
      end
    ...
    def valid?
      ...
      return false if @i_d_code.to_s.length > 6
    ...
    def i_d_code=(i_d_code)
      ...
      if i_d_code.to_s.length > 6
        fail ArgumentError, 'invalid value for "i_d_code", the character length must be smaller than or equal to 6.'
      end
    ...
    def hash
      [group_id, division_id, i_d_code, description, entry_date, entry_user, update_date, update_user].hash
    end
