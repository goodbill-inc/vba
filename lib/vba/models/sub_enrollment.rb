  ...
  class SubEnrollment
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::SubEnrollment` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::SubEnrollment`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@entry_user.nil? && @entry_user.to_s.length > 50
        invalid_properties.push('invalid value for "entry_user", the character length must be smaller than or equal to 50.')
      end
      ...
      if !@update_user.nil? && @update_user.to_s.length > 50
        invalid_properties.push('invalid value for "update_user", the character length must be smaller than or equal to 50.')
      end
    ...
    def valid?
      ...
      return false if !@entry_user.nil? && @entry_user.to_s.length > 50
      return false if !@policy_number.nil? && @policy_number.to_s.length > 25
      return false if !@update_user.nil? && @update_user.to_s.length > 50
      true
    end
    ...
    def entry_user=(entry_user)
      if !entry_user.nil? && entry_user.to_s.length > 50
        fail ArgumentError, 'invalid value for "entry_user", the character length must be smaller than or equal to 50.'
      end
    ...
    def update_user=(update_user)
      if !update_user.nil? && update_user.to_s.length > 50
        fail ArgumentError, 'invalid value for "update_user", the character length must be smaller than or equal to 50.'
      end
    ...
    def hash
      [subscriber_id, group_id, division_id, group_coverage_start, start_date, annual_max, date_employed, eligibility_date, end_date, entry_date, entry_user, lifetime_max, original_enrollment_date, paid_through, policy_number, term_date, update_date, update_user].hash
    end
