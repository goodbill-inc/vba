...
module Vba
  class ClaimDetail
    # References the Claim.Claim_Number value on the parent record. Note: This field is marked as NOT NULL in the database and therefore required by the API.
    ...
    # Notes if this line was adjusted as part of an individual claim adjustment process. Note: This field is marked as NOT NULL in the database and therefore required by the API.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::ClaimDetail` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::ClaimDetail`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'adjusted_line')
        self.adjusted_line = attributes[:'adjusted_line']
      else
        self.adjusted_line = nil
      end
    ...
    def list_invalid_properties
      ...
      if @adjusted_line.nil?
        invalid_properties.push('invalid value for "adjusted_line", adjusted_line cannot be nil.')
      end

      if @admin_amt.nil?
        invalid_properties.push('invalid value for "admin_amt", admin_amt cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @adjusted_line.nil?
      return false if @admin_amt.nil?
    ...
    # Custom attribute writer method with validation
    # @param [Object] adjusted_line Value to be assigned
    def adjusted_line=(adjusted_line)
      if adjusted_line.nil?
        fail ArgumentError, 'adjusted_line cannot be nil'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] admin_amt Value to be assigned
    def admin_amt=(admin_amt)
      if admin_amt.nil?
        fail ArgumentError, 'admin_amt cannot be nil'
      end
    ...
    def hash
      [claim_number, claim_seq, adjustment_seq, ans_i_code, ap_c_code, account_key, adjusted_line, admin_amt, auth_number, auth_penalty, auth_required, benefit_code, benefit_rate_applied, billed_price, brand_indicator, buccal, cli_a_number, co_b_amt, co_b_hold, co_b_mem_resp, co_b_savings, co_b_savings_used, calendar_year, capitation_claim, check_id, co_ins_amt, co_pay_amt, currency_id, days_supply, ded_credit_amt, deductible_amt, detail_plan_year, diagnosis_code, discount_amt, dispense_as_written, distal, drug_code, drug_quantity, entry_date, entry_user, ex_code, ex_code2, ex_code3, fica, facial, federal_amount_to_tax, federal_tax, federal_tax_flat, fill_date, flex_description, formulary_indicator, incisal, interest, labial, life_benefit, life_benefit_volume, line_control_number, lingual, local_tax, mail_order_indicator, medicare_amount_to_tax, medicare_tax, mesial, modifier, modifier_2, modifier_3, modifier_4, not_covered_amt, occlusal, oral_cavity_designation_code_1, oral_cavity_designation_code_2, oral_cavity_designation_code_3, oral_cavity_designation_code_4, oral_cavity_designation_code_5, other_deduction_amount, override_benefit_code, override_cob_amt, override_co_ins_amt, override_co_pay_amt, override_ded_credit_amt, override_deductible_amt, override_federal_tax, override_local_tax, override_medicare_tax, override_not_covered_amt, override_plan_price, override_social_security, override_state_tax, override_with_hold_amt, pp_o_price_plan_price_diff, paid_amt, partial_loss, payor_id, pharmacy_name, place_of_service, plan_price, premium_deduction, prescriber_id, prescription_date, prescription_number, pricing_method, pricing_rate, principal, procedure_code, procedure_code2, qualifying_payment_amount, reason_code, refill_indicator, repriced_savings_amount, root, service_date, service_thru, soc_sec_amount_to_tax, social_security, state_amount_to_tax, state_tax, state_tax_flat, state_tax_state, status_code, svc_count, taxable_amount, tooth_number, uc_r_price, unit_dose, unit_of_measure, update_date, update_user, with_hold_amt].hash
    end
