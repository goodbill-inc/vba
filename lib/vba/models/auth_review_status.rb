...
module Vba
  class AuthReviewStatus
    # Name of the review status.
    attr_accessor :review_status

    # Description of the associated review status.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::AuthReviewStatus` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::AuthReviewStatus`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @review_status.to_s.length > 32
        invalid_properties.push('invalid value for "review_status", the character length must be smaller than or equal to 32.')
      end

      if @review_status.to_s.length < 1
        invalid_properties.push('invalid value for "review_status", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @review_status.to_s.length > 32
      return false if @review_status.to_s.length < 1
    ...
    def review_status=(review_status)
      ...
      if review_status.to_s.length < 1
        fail ArgumentError, 'invalid value for "review_status", the character length must be great than or equal to 1.'
      end
    ...
    def hash
      [review_status, description, entry_date, entry_user, update_date, update_user].hash
    end
