  ...
  class ZipCodes
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::ZipCodes` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::ZipCodes`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @zip_code.to_s.length > 32
        invalid_properties.push('invalid value for "zip_code", the character length must be smaller than or equal to 32.')
      end

      if @zip_code.to_s.length < 1
        invalid_properties.push('invalid value for "zip_code", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @zip_code.to_s.length > 32
      return false if @zip_code.to_s.length < 1
    ...
    def zip_code=(zip_code)
      ...
      if zip_code.to_s.length < 1
        fail ArgumentError, 'invalid value for "zip_code", the character length must be great than or equal to 1.'
      end
    ...
    def hash
      [zip_code, city, country_id, county, entry_date, entry_user, state, update_date, update_user].hash
    end
