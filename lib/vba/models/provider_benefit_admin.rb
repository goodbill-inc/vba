  ...
  class ProviderBenefitAdmin
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::ProviderBenefitAdmin` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::ProviderBenefitAdmin`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'admin_column')
        self.admin_column = attributes[:'admin_column']
      end
    ...
    def list_invalid_properties
      ...
      if !@admin_column.nil? && @admin_column.to_s.length > 64
        invalid_properties.push('invalid value for "admin_column", the character length must be smaller than or equal to 64.')
      end

      if @admin_pct.nil?
        invalid_properties.push('invalid value for "admin_pct", admin_pct cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if !@admin_column.nil? && @admin_column.to_s.length > 64
      return false if @admin_pct.nil?
    ...
    # Custom attribute writer method with validation
    # @param [Object] admin_column Value to be assigned
    def admin_column=(admin_column)
      if !admin_column.nil? && admin_column.to_s.length > 64
        fail ArgumentError, 'invalid value for "admin_column", the character length must be smaller than or equal to 64.'
      end
    ...
    def admin_pct=(admin_pct)
      if admin_pct.nil?
        fail ArgumentError, 'admin_pct cannot be nil'
      end
    ...
    def hash
      [provider_id, benefit_code, effective_date, admin_column, admin_fee, admin_pct, entry_date, entry_user, network_id, term_date, update_date, update_user].hash
    end
