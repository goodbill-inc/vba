...
module Vba
  class GetSubscriberDisenrollAdjustmentsResult
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::GetSubscriberDisenrollAdjustmentsResult` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::GetSubscriberDisenrollAdjustmentsResult`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [selected, invoice_key, invoice_number, invoice_date, billing_from, billing_thru, group_id, division_id, prem_rate_key, rate_type, rate_description, invoice_days, pro_rate_days, invoice_amount, invoice_received, invoice_amount_per_day, pro_rate_amount, adjustment_amount, auto_adjust, assign_invoice_key].hash
    end
