  ...
  class GroupPriceAdjust
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::GroupPriceAdjust` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::GroupPriceAdjust`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'price_adjust')
        self.price_adjust = attributes[:'price_adjust']
      end
    ...
    def hash
      [group_id, effective_date, entry_date, entry_user, price_adjust, term_date, update_date, update_user].hash
    end
