...
module Vba
  class FundingAccountType
    # Name of the 'Funding Account Type'.
    attr_accessor :account_type

    # Description of the 'Funding Account Type'.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::FundingAccountType` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::FundingAccountType`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if @account_type.to_s.length > 8
      return false if @account_type.to_s.length < 1
    ...
    def account_type=(account_type)
      ...
      if account_type.to_s.length < 1
        fail ArgumentError, 'invalid value for "account_type", the character length must be great than or equal to 1.'
      end
    ...
    def hash
      [account_type, description, entry_date, entry_user, update_date, update_user].hash
    end
