...
module Vba
  class MaritalStatus
    # Identifier for the marital status
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::MaritalStatus` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::MaritalStatus`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @marital_status.to_s.length > 1
        invalid_properties.push('invalid value for "marital_status", the character length must be smaller than or equal to 1.')
      end

      if @marital_status.to_s.length < 1
        invalid_properties.push('invalid value for "marital_status", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @marital_status.to_s.length > 1
      return false if @marital_status.to_s.length < 1
    ...
    def marital_status=(marital_status)
      if marital_status.nil?
        fail ArgumentError, 'marital_status cannot be nil'
      end
      ...
      if marital_status.to_s.length < 1
        fail ArgumentError, 'invalid value for "marital_status", the character length must be great than or equal to 1.'
      end
    ...
    def hash
      [marital_status, description].hash
    end
