...
module Vba
  class DiagnosticCodeType
    # Name for the diagnostic code type.
    attr_accessor :diagnostic_code_type

    # Description of the diagnostic code type.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::DiagnosticCodeType` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::DiagnosticCodeType`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @diagnostic_code_type.to_s.length > 8
        invalid_properties.push('invalid value for "diagnostic_code_type", the character length must be smaller than or equal to 8.')
      end
    ...
    def valid?
      ...
      return false if @diagnostic_code_type.to_s.length > 8
    ...
    def diagnostic_code_type=(diagnostic_code_type)
      ...
      if diagnostic_code_type.to_s.length > 8
        fail ArgumentError, 'invalid value for "diagnostic_code_type", the character length must be smaller than or equal to 8.'
      end
    ...
    def hash
      [diagnostic_code_type, description].hash
    end
