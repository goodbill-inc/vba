  ...
  class AuthAction
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::AuthAction` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::AuthAction`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @auth_action.nil?
        invalid_properties.push('invalid value for "auth_action", auth_action cannot be nil.')
      end

      if @auth_action.to_s.length > 1
        invalid_properties.push('invalid value for "auth_action", the character length must be smaller than or equal to 1.')
      end

      if @auth_action.to_s.length < 1
        invalid_properties.push('invalid value for "auth_action", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @auth_action.nil?
      return false if @auth_action.to_s.length > 1
      return false if @auth_action.to_s.length < 1
    ...
    def auth_action=(auth_action)
      if auth_action.nil?
        fail ArgumentError, 'auth_action cannot be nil'
      end
      ...
      if auth_action.to_s.length < 1
        fail ArgumentError, 'invalid value for "auth_action", the character length must be great than or equal to 1.'
      end
