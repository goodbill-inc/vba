...
module Vba
  class MemberPreExDiag
    ...
    # Identifies the ICD version of the associated diagnosis codes. Note: This field is marked as NOT NULL in the database and therefore required by the API.
    attr_accessor :diagnostic_code_type

    # Identifies the beginning diagnosis code for the associated range during the Member's preexisting condition period.
    ...
    # Identifies the ending diagnosis code for the associated range during the Member's preexisting condition period.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::MemberPreExDiag` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::MemberPreExDiag`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [subscriber_id, member_seq, diagnostic_code_type, from_diag, entry_date, entry_user, thru_diag, update_date, update_user].hash
    end
