  ...
  class ProcedureCodeSet
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::ProcedureCodeSet` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::ProcedureCodeSet`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @code_set.nil?
        invalid_properties.push('invalid value for "code_set", code_set cannot be nil.')
      end

      if @code_set.to_s.length > 6
        invalid_properties.push('invalid value for "code_set", the character length must be smaller than or equal to 6.')
      end

      if @code_set.to_s.length < 1
        invalid_properties.push('invalid value for "code_set", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @code_set.nil?
      return false if @code_set.to_s.length > 6
      return false if @code_set.to_s.length < 1
    ...
    # Custom attribute writer method with validation
    # @param [Object] code_set Value to be assigned
    def code_set=(code_set)
      if code_set.nil?
        fail ArgumentError, 'code_set cannot be nil'
      end

      if code_set.to_s.length > 6
        fail ArgumentError, 'invalid value for "code_set", the character length must be smaller than or equal to 6.'
      end

      if code_set.to_s.length < 1
        fail ArgumentError, 'invalid value for "code_set", the character length must be great than or equal to 1.'
      end
    ...
    def hash
      [procedure_code, code_set, description, entry_date, entry_user, update_date, update_user].hash
    end
