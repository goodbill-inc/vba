...
module Vba
  class PlanAddedAccumBenefits
    # References the PlanAddedAccum.Plan_ID for this Added Accumulator.
    attr_accessor :plan_id

    # References the PlanAddedAccum.Accum_Name for this Added Accumulator.
    attr_accessor :accum_name

    # References the BenefitCodes.Benefit_Code for this Added Accumulator.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::PlanAddedAccumBenefits` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::PlanAddedAccumBenefits`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [plan_id, accum_name, benefit_code].hash
    end
