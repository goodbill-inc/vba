...
module Vba
  class ClaimInvoiceDetail
    # Invoice the claim is on Note: This field is marked as NOT NULL in the database and therefore required by the API.
    attr_accessor :invoice_key

    # Unique identifier for the Batch Number Note: This field is marked as NOT NULL in the database and therefore required by the API.
    attr_accessor :batch_number

    # Unique identifier for the Batch Claim Note: This field is marked as NOT NULL in the database and therefore required by the API.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::ClaimInvoiceDetail` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::ClaimInvoiceDetail`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [invoice_key, batch_number, batch_claim, entry_date, entry_user, update_date, update_user].hash
    end
