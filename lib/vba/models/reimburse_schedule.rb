  ...
  class ReimburseSchedule
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::ReimburseSchedule` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::ReimburseSchedule`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @reimburse_schedule.nil?
        invalid_properties.push('invalid value for "reimburse_schedule", reimburse_schedule cannot be nil.')
      end

      if @reimburse_schedule.to_s.length > 32
        invalid_properties.push('invalid value for "reimburse_schedule", the character length must be smaller than or equal to 32.')
      end

      if @reimburse_schedule.to_s.length < 1
        invalid_properties.push('invalid value for "reimburse_schedule", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @reimburse_schedule.nil?
      return false if @reimburse_schedule.to_s.length > 32
      return false if @reimburse_schedule.to_s.length < 1
    ...
    def reimburse_schedule=(reimburse_schedule)
      if reimburse_schedule.nil?
        fail ArgumentError, 'reimburse_schedule cannot be nil'
      end
      ...
      if reimburse_schedule.to_s.length < 1
        fail ArgumentError, 'invalid value for "reimburse_schedule", the character length must be great than or equal to 1.'
      end
    ...
    def hash
      [reimburse_schedule, description, entry_date, entry_user, schedule_frequency, schedule_type, update_date, update_user].hash
    end
