...
module Vba
  class ReinsContractMemberSpecific
    # Refernces the ReinsContract.ReinsContract_Key on parent record. Each value is a laser deductible for a member on this contract. Note: This field is marked as NOT NULL in the database and therefore required by the API.
    attr_accessor :reins_contract_key

    # The member that is identified for this laser deductible.
    ...
    # The lasered deductible to be carved out within this contract.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::ReinsContractMemberSpecific` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::ReinsContractMemberSpecific`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [reins_contract_key, subscriber_id, member_seq, entry_date, entry_user, specific_deductible, update_date, update_user].hash
    end
