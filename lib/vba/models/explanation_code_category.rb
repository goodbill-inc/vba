  ...
  class ExplanationCodeCategory
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::ExplanationCodeCategory` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::ExplanationCodeCategory`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if @category.to_s.length > 6
    ...
    # Custom attribute writer method with validation
    # @param [Object] invalid Value to be assigned
    def invalid=(invalid)
      if invalid.nil?
        fail ArgumentError, 'invalid cannot be nil'
      end
    ...
    def hash
      [category, description, entry_date, entry_user, invalid, update_date, update_user].hash
    end
