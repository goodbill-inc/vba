...
module Vba
  class GroupACHPull
    ...
    # Day the ACH transaction occurs Note: This field is marked as NOT NULL in the database and therefore required by the API.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::GroupACHPull` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::GroupACHPull`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @ac_h_pull_day.nil?
        invalid_properties.push('invalid value for "ac_h_pull_day", ac_h_pull_day cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @ac_h_pull_day.nil?
    ...
    # Custom attribute writer method with validation
    # @param [Object] ac_h_pull_day Value to be assigned
    def ac_h_pull_day=(ac_h_pull_day)
      if ac_h_pull_day.nil?
        fail ArgumentError, 'ac_h_pull_day cannot be nil'
      end

      @ac_h_pull_day = ac_h_pull_day
    end
    ...
    def hash
      [group_id, ac_h_pull_day, entry_date, entry_user, update_date, update_user].hash
    end
