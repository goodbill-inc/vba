  ...
  class ClaimDetailExCode
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::ClaimDetailExCode` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::ClaimDetailExCode`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'ex_code_source')
        self.ex_code_source = attributes[:'ex_code_source']
      end
    ...
    def list_invalid_properties
      ...
      if !@ex_code_source.nil? && @ex_code_source.to_s.length > 16
        invalid_properties.push('invalid value for "ex_code_source", the character length must be smaller than or equal to 16.')
      end
    ...
    def valid?
      ...
      return false if !@ex_code_source.nil? && @ex_code_source.to_s.length > 16
    ...
    # Custom attribute writer method with validation
    # @param [Object] cleared Value to be assigned
    def cleared=(cleared)
      if cleared.nil?
        fail ArgumentError, 'cleared cannot be nil'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] ex_code_source Value to be assigned
    def ex_code_source=(ex_code_source)
      if !ex_code_source.nil? && ex_code_source.to_s.length > 16
        fail ArgumentError, 'invalid value for "ex_code_source", the character length must be smaller than or equal to 16.'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          ex_code_source == o.ex_code_source &&
    ...
    def hash
      [batch_number, batch_claim, claim_seq, adjustment_seq, ex_code, claim_rule_key, cleared, comment, entry_date, entry_user, ex_code_source, update_date, update_user].hash
    end
