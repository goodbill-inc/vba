=begin
#VBASoftware

#APIs for VBASoftware

The version of the OpenAPI document: v1

Generated by: https://openapi-generator.tech
Generator version: 7.12.0

=end

require 'date'
require 'time'

module Vba
  class FundingNameOnCheck
    attr_accessor :send_check_to

    attr_accessor :pay_to_id

    attr_accessor :subscriber_id

    attr_accessor :member_seq

    attr_accessor :name_on_check

    attr_accessor :address_on_check

    attr_accessor :address2_on_check

    attr_accessor :city_on_check

    attr_accessor :state_on_check

    attr_accessor :zip_on_check

    attr_accessor :country_id_on_check

    attr_accessor :name_on_address

    # Attribute mapping from ruby-style variable name to JSON key.
    def self.attribute_map
      {
        :'send_check_to' => :'send_Check_To',
        :'pay_to_id' => :'pay_To_ID',
        :'subscriber_id' => :'subscriber_ID',
        :'member_seq' => :'member_Seq',
        :'name_on_check' => :'name_On_Check',
        :'address_on_check' => :'address_On_Check',
        :'address2_on_check' => :'address2_On_Check',
        :'city_on_check' => :'city_On_Check',
        :'state_on_check' => :'state_On_Check',
        :'zip_on_check' => :'zip_On_Check',
        :'country_id_on_check' => :'country_ID_On_Check',
        :'name_on_address' => :'name_On_Address'
      }
    end

    # Returns attribute mapping this model knows about
    def self.acceptable_attribute_map
      attribute_map
    end

    # Returns all the JSON keys this model knows about
    def self.acceptable_attributes
      acceptable_attribute_map.values
    end

    # Attribute type mapping.
    def self.openapi_types
      {
        :'send_check_to' => :'String',
        :'pay_to_id' => :'String',
        :'subscriber_id' => :'String',
        :'member_seq' => :'String',
        :'name_on_check' => :'String',
        :'address_on_check' => :'String',
        :'address2_on_check' => :'String',
        :'city_on_check' => :'String',
        :'state_on_check' => :'String',
        :'zip_on_check' => :'String',
        :'country_id_on_check' => :'String',
        :'name_on_address' => :'String'
      }
    end

    # List of attributes with nullable: true
    def self.openapi_nullable
      Set.new([
        :'subscriber_id',
        :'member_seq',
        :'name_on_check',
        :'address_on_check',
        :'address2_on_check',
        :'city_on_check',
        :'state_on_check',
        :'zip_on_check',
        :'country_id_on_check',
        :'name_on_address'
      ])
    end

    # Initializes the object
    # @param [Hash] attributes Model attributes in the form of hash
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::FundingNameOnCheck` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      acceptable_attribute_map = self.class.acceptable_attribute_map
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::FundingNameOnCheck`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'send_check_to')
        self.send_check_to = attributes[:'send_check_to']
      else
        self.send_check_to = nil
      end

      if attributes.key?(:'pay_to_id')
        self.pay_to_id = attributes[:'pay_to_id']
      else
        self.pay_to_id = nil
      end

      if attributes.key?(:'subscriber_id')
        self.subscriber_id = attributes[:'subscriber_id']
      end

      if attributes.key?(:'member_seq')
        self.member_seq = attributes[:'member_seq']
      end

      if attributes.key?(:'name_on_check')
        self.name_on_check = attributes[:'name_on_check']
      end

      if attributes.key?(:'address_on_check')
        self.address_on_check = attributes[:'address_on_check']
      end

      if attributes.key?(:'address2_on_check')
        self.address2_on_check = attributes[:'address2_on_check']
      end

      if attributes.key?(:'city_on_check')
        self.city_on_check = attributes[:'city_on_check']
      end

      if attributes.key?(:'state_on_check')
        self.state_on_check = attributes[:'state_on_check']
      end

      if attributes.key?(:'zip_on_check')
        self.zip_on_check = attributes[:'zip_on_check']
      end

      if attributes.key?(:'country_id_on_check')
        self.country_id_on_check = attributes[:'country_id_on_check']
      end

      if attributes.key?(:'name_on_address')
        self.name_on_address = attributes[:'name_on_address']
      end
    end

    # Show invalid properties with the reasons. Usually used together with valid?
    # @return Array for valid properties with the reasons
    def list_invalid_properties
      warn '[DEPRECATED] the `list_invalid_properties` method is obsolete'
      invalid_properties = Array.new
      if @send_check_to.nil?
        invalid_properties.push('invalid value for "send_check_to", send_check_to cannot be nil.')
      end

      if @send_check_to.to_s.length < 1
        invalid_properties.push('invalid value for "send_check_to", the character length must be great than or equal to 1.')
      end

      if @pay_to_id.nil?
        invalid_properties.push('invalid value for "pay_to_id", pay_to_id cannot be nil.')
      end

      if @pay_to_id.to_s.length < 1
        invalid_properties.push('invalid value for "pay_to_id", the character length must be great than or equal to 1.')
      end

      invalid_properties
    end

    # Check to see if the all the properties in the model are valid
    # @return true if the model is valid
    def valid?
      warn '[DEPRECATED] the `valid?` method is obsolete'
      return false if @send_check_to.nil?
      return false if @send_check_to.to_s.length < 1
      return false if @pay_to_id.nil?
      return false if @pay_to_id.to_s.length < 1
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] send_check_to Value to be assigned
    def send_check_to=(send_check_to)
      if send_check_to.nil?
        fail ArgumentError, 'send_check_to cannot be nil'
      end

      if send_check_to.to_s.length < 1
        fail ArgumentError, 'invalid value for "send_check_to", the character length must be great than or equal to 1.'
      end

      @send_check_to = send_check_to
    end

    # Custom attribute writer method with validation
    # @param [Object] pay_to_id Value to be assigned
    def pay_to_id=(pay_to_id)
      if pay_to_id.nil?
        fail ArgumentError, 'pay_to_id cannot be nil'
      end

      if pay_to_id.to_s.length < 1
        fail ArgumentError, 'invalid value for "pay_to_id", the character length must be great than or equal to 1.'
      end

      @pay_to_id = pay_to_id
    end

    # Checks equality by comparing each attribute.
    # @param [Object] Object to be compared
    def ==(o)
      return true if self.equal?(o)
      self.class == o.class &&
          send_check_to == o.send_check_to &&
          pay_to_id == o.pay_to_id &&
          subscriber_id == o.subscriber_id &&
          member_seq == o.member_seq &&
          name_on_check == o.name_on_check &&
          address_on_check == o.address_on_check &&
          address2_on_check == o.address2_on_check &&
          city_on_check == o.city_on_check &&
          state_on_check == o.state_on_check &&
          zip_on_check == o.zip_on_check &&
          country_id_on_check == o.country_id_on_check &&
          name_on_address == o.name_on_address
    end

    # @see the `==` method
    # @param [Object] Object to be compared
    def eql?(o)
      self == o
    end

    # Calculates hash code according to all attributes.
    # @return [Integer] Hash code
    def hash
      [send_check_to, pay_to_id, subscriber_id, member_seq, name_on_check, address_on_check, address2_on_check, city_on_check, state_on_check, zip_on_check, country_id_on_check, name_on_address].hash
    end

    # Builds the object from hash
    # @param [Hash] attributes Model attributes in the form of hash
    # @return [Object] Returns the model itself
    def self.build_from_hash(attributes)
      return nil unless attributes.is_a?(Hash)
      attributes = attributes.transform_keys(&:to_sym)
      transformed_hash = {}
      openapi_types.each_pair do |key, type|
        if attributes.key?(attribute_map[key]) && attributes[attribute_map[key]].nil?
          transformed_hash["#{key}"] = nil
        elsif type =~ /\AArray<(.*)>/i
          # check to ensure the input is an array given that the attribute
          # is documented as an array but the input is not
          if attributes[attribute_map[key]].is_a?(Array)
            transformed_hash["#{key}"] = attributes[attribute_map[key]].map { |v| _deserialize($1, v) }
          end
        elsif !attributes[attribute_map[key]].nil?
          transformed_hash["#{key}"] = _deserialize(type, attributes[attribute_map[key]])
        end
      end
      new(transformed_hash)
    end

    # Deserializes the data based on type
    # @param string type Data type
    # @param string value Value to be deserialized
    # @return [Object] Deserialized data
    def self._deserialize(type, value)
      case type.to_sym
      when :Time
        Time.parse(value)
      when :Date
        Date.parse(value)
      when :String
        value.to_s
      when :Integer
        value.to_i
      when :Float
        value.to_f
      when :Boolean
        if value.to_s =~ /\A(true|t|yes|y|1)\z/i
          true
        else
          false
        end
      when :Object
        # generic object (usually a Hash), return directly
        value
      when /\AArray<(?<inner_type>.+)>\z/
        inner_type = Regexp.last_match[:inner_type]
        value.map { |v| _deserialize(inner_type, v) }
      when /\AHash<(?<k_type>.+?), (?<v_type>.+)>\z/
        k_type = Regexp.last_match[:k_type]
        v_type = Regexp.last_match[:v_type]
        {}.tap do |hash|
          value.each do |k, v|
            hash[_deserialize(k_type, k)] = _deserialize(v_type, v)
          end
        end
      else # model
        # models (e.g. Pet) or oneOf
        klass = Vba.const_get(type)
        klass.respond_to?(:openapi_any_of) || klass.respond_to?(:openapi_one_of) ? klass.build(value) : klass.build_from_hash(value)
      end
    end

    # Returns the string representation of the object
    # @return [String] String presentation of the object
    def to_s
      to_hash.to_s
    end

    # to_body is an alias to to_hash (backward compatibility)
    # @return [Hash] Returns the object in the form of hash
    def to_body
      to_hash
    end

    # Returns the object in the form of hash
    # @return [Hash] Returns the object in the form of hash
    def to_hash
      hash = {}
      self.class.attribute_map.each_pair do |attr, param|
        value = self.send(attr)
        if value.nil?
          is_nullable = self.class.openapi_nullable.include?(attr)
          next if !is_nullable || (is_nullable && !instance_variable_defined?(:"@#{attr}"))
        end

        hash[param] = _to_hash(value)
      end
      hash
    end

    # Outputs non-array value in the form of hash
    # For object, use to_hash. Otherwise, just return the value
    # @param [Object] value Any valid value
    # @return [Hash] Returns the value in the form of hash
    def _to_hash(value)
      if value.is_a?(Array)
        value.compact.map { |v| _to_hash(v) }
      elsif value.is_a?(Hash)
        {}.tap do |hash|
          value.each { |k, v| hash[k] = _to_hash(v) }
        end
      elsif value.respond_to? :to_hash
        value.to_hash
      else
        value
      end
    end

  end

end
