...
module Vba
  class MemberAccountPlan
    # Unique identifier for each Plan and Account combination for this Member. Note: This field is marked as NOT NULL in the database and therefore required by the API.
    attr_accessor :member_account_key

    # References the SubenrollmentPlan.Plan_ID value for this members enrollment.
    attr_accessor :plan_id

    # Total Benefit amount associated with the Member's plan.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::MemberAccountPlan` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::MemberAccountPlan`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'benefit_amount')
        self.benefit_amount = attributes[:'benefit_amount']
      end
    ...
    def hash
      [member_account_key, plan_id, benefit_amount, entry_date, entry_user, update_date, update_user].hash
    end
