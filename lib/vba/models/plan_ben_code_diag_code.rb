  ...
  class PlanBenCodeDiagCode
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::PlanBenCodeDiagCode` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::PlanBenCodeDiagCode`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @from_diag_code.nil?
        invalid_properties.push('invalid value for "from_diag_code", from_diag_code cannot be nil.')
      end

      if @from_diag_code.to_s.length > 8
        invalid_properties.push('invalid value for "from_diag_code", the character length must be smaller than or equal to 8.')
      end

      if @from_diag_code.to_s.length < 1
        invalid_properties.push('invalid value for "from_diag_code", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @from_diag_code.nil?
      return false if @from_diag_code.to_s.length > 8
      return false if @from_diag_code.to_s.length < 1
    ...
    def from_diag_code=(from_diag_code)
      if from_diag_code.nil?
        fail ArgumentError, 'from_diag_code cannot be nil'
      end
      ...
      if from_diag_code.to_s.length < 1
        fail ArgumentError, 'invalid value for "from_diag_code", the character length must be great than or equal to 1.'
      end
    ...
    def hash
      [plan_id, benefit_code, diagnostic_code_type, from_diag_code, entry_date, entry_user, exclude, thru_diag_code, update_date, update_user].hash
    end
