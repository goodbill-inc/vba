...
module Vba
  class CheckEdit
    # ID of the check edit.
    attr_accessor :check_edit_key

    # PB object used by the check edit to process.
    ...
    # PB object used by the check edit to display results.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::CheckEdit` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::CheckEdit`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'result_object')
        self.result_object = attributes[:'result_object']
      end
    end
    ...
    def list_invalid_properties
      ...
      if !@result_object.nil? && @result_object.to_s.length > 64
        invalid_properties.push('invalid value for "result_object", the character length must be smaller than or equal to 64.')
      end
    ...
    def valid?
      ...
      return false if !@result_object.nil? && @result_object.to_s.length > 64
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] result_object Value to be assigned
    def result_object=(result_object)
      if !result_object.nil? && result_object.to_s.length > 64
        fail ArgumentError, 'invalid value for "result_object", the character length must be smaller than or equal to 64.'
      end
    ...
    def hash
      [check_edit_key, check_edit_object, description, display, result_object].hash
    end
