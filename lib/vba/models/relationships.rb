  ...
  class Relationships
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::Relationships` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::Relationships`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@spouse.nil? && @spouse.to_s.length > 1
        invalid_properties.push('invalid value for "spouse", the character length must be smaller than or equal to 1.')
      end

      if !@subscriber.nil? && @subscriber.to_s.length > 1
        invalid_properties.push('invalid value for "subscriber", the character length must be smaller than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if !@spouse.nil? && @spouse.to_s.length > 1
      return false if !@subscriber.nil? && @subscriber.to_s.length > 1
    ...
    def spouse=(spouse)
      if !spouse.nil? && spouse.to_s.length > 1
        fail ArgumentError, 'invalid value for "spouse", the character length must be smaller than or equal to 1.'
      end
    ...
    def subscriber=(subscriber)
      if !subscriber.nil? && subscriber.to_s.length > 1
        fail ArgumentError, 'invalid value for "subscriber", the character length must be smaller than or equal to 1.'
      end
    ...
    def hash
      [relate_code, description, entry_date, entry_user, hipa_a_code, spouse, subscriber, update_date, update_user].hash
    end
