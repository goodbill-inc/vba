...
module Vba
  class MemberToothExCode
    ...
    # Identifies the number of the tooth/teeth that the User intends to apply an Ex Code to. Note: This field is marked as NOT NULL in the database and therefore required by the API.
    ...
    # Idenfities the Ex Code that will be applied to the selected tooth/teeth.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::MemberToothExCode` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::MemberToothExCode`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if @ex_code.to_s.length > 8
    ...
    def hash
      [subscriber_id, member_seq, tooth_number, entry_date, entry_user, ex_code, update_date, update_user].hash
    end
