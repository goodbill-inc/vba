...
module Vba
  class MemberBeneficiaryPlan
    # References MemberBeneficiary.Subscriber_ID value on the parent record.
    attr_accessor :subscriber_id

    # References MemberBeneficiary.Member_Seq value on the parent record.
    attr_accessor :member_seq

    # References MemberBeneficiary.Beneficiary_ID value on the parent record.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::MemberBeneficiaryPlan` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::MemberBeneficiaryPlan`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [subscriber_id, member_seq, beneficiary_id, plan_id, entry_date, entry_user, update_date, update_user].hash
    end
