  ...
  class ClaimCodingRuleType
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::ClaimCodingRuleType` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::ClaimCodingRuleType`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @rule_type.to_s.length > 16
        invalid_properties.push('invalid value for "rule_type", the character length must be smaller than or equal to 16.')
      end

      if @rule_type.to_s.length < 1
        invalid_properties.push('invalid value for "rule_type", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @rule_type.to_s.length > 16
      return false if @rule_type.to_s.length < 1
    ...
    def rule_type=(rule_type)
      ...
      if rule_type.to_s.length < 1
        fail ArgumentError, 'invalid value for "rule_type", the character length must be great than or equal to 1.'
      end
    ...
    def hash
      [rule_type, description, display, entry_date, entry_user, update_date, update_user].hash
    end
