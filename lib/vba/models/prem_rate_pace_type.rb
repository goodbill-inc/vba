  ...
  class PremRatePaceType
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::PremRatePaceType` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::PremRatePaceType`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @rate_pace_type.nil?
        invalid_properties.push('invalid value for "rate_pace_type", rate_pace_type cannot be nil.')
      end

      if @rate_pace_type.to_s.length > 16
        invalid_properties.push('invalid value for "rate_pace_type", the character length must be smaller than or equal to 16.')
      end

      if @rate_pace_type.to_s.length < 1
        invalid_properties.push('invalid value for "rate_pace_type", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @rate_pace_type.nil?
      return false if @rate_pace_type.to_s.length > 16
      return false if @rate_pace_type.to_s.length < 1
    ...
    def rate_pace_type=(rate_pace_type)
      if rate_pace_type.nil?
        fail ArgumentError, 'rate_pace_type cannot be nil'
      end
      ...
      if rate_pace_type.to_s.length < 1
        fail ArgumentError, 'invalid value for "rate_pace_type", the character length must be great than or equal to 1.'
      end
    ...
    def hash
      [rate_pace_type, description, entry_date, entry_user, rate_pace, sort_order, update_date, update_user].hash
    end
