...
module Vba
  class PlanBenefitPBM
    ...
    # User-defined type of pharmacy benefit copay. This information is pulled into the Member SPD matrix on the Member screen.
    attr_accessor :pb_m_type

    # Amount of Copay that the Member is responsible for.
    attr_accessor :co_pay_amount
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::PlanBenefitPBM` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::PlanBenefitPBM`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'pb_m_type')
        self.pb_m_type = attributes[:'pb_m_type']
      else
        self.pb_m_type = nil
      end

      if attributes.key?(:'co_pay_amount')
        self.co_pay_amount = attributes[:'co_pay_amount']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @pb_m_type.nil?
        invalid_properties.push('invalid value for "pb_m_type", pb_m_type cannot be nil.')
      end

      if @pb_m_type.to_s.length > 8
        invalid_properties.push('invalid value for "pb_m_type", the character length must be smaller than or equal to 8.')
      end

      if @pb_m_type.to_s.length < 1
        invalid_properties.push('invalid value for "pb_m_type", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @pb_m_type.nil?
      return false if @pb_m_type.to_s.length > 8
      return false if @pb_m_type.to_s.length < 1
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] pb_m_type Value to be assigned
    def pb_m_type=(pb_m_type)
      if pb_m_type.nil?
        fail ArgumentError, 'pb_m_type cannot be nil'
      end

      if pb_m_type.to_s.length > 8
        fail ArgumentError, 'invalid value for "pb_m_type", the character length must be smaller than or equal to 8.'
      end

      if pb_m_type.to_s.length < 1
        fail ArgumentError, 'invalid value for "pb_m_type", the character length must be great than or equal to 1.'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          pb_m_type == o.pb_m_type &&
          co_pay_amount == o.co_pay_amount
    end
    ...
    def hash
      [plan_id, benefit_code, pb_m_type, co_pay_amount].hash
    end
