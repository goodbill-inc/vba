...
module Vba
  class GroupContractPlanRiderExternalID
    # References GroupContractPlanRider.Group_ID value on the parent record.
    attr_accessor :group_id

    # References GroupContractPlanRider.Division_ID value on the parent record.
    attr_accessor :division_id

    # References GroupContractPlanRider.Coverage_Start value on the parent record. Note: This field is marked as NOT NULL in the database and therefore required by the API.
    attr_accessor :coverage_start

    # References GroupContractPlanRider.Plan_ID value on the parent record.
    attr_accessor :plan_id

    # References GroupContractPlanRider.Benefit_Code value on the parent record.
    attr_accessor :benefit_code

    # The external ID used for custom tax reporting integrations.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::GroupContractPlanRiderExternalID` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::GroupContractPlanRiderExternalID`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @external_id.to_s.length > 32
        invalid_properties.push('invalid value for "external_id", the character length must be smaller than or equal to 32.')
      end
    ...
    def valid?
      ...
      return false if @external_id.to_s.length > 32
    ...
    def external_id=(external_id)
      ...
      if external_id.to_s.length > 32
        fail ArgumentError, 'invalid value for "external_id", the character length must be smaller than or equal to 32.'
      end
    ...
    def hash
      [group_id, division_id, coverage_start, plan_id, benefit_code, external_id, entry_date, entry_user, update_date, update_user].hash
    end
