...
module Vba
  class CallTrackingSubscriber
    # ID assigned to the call.
    attr_accessor :call_key

    # ID of the Subscriber associated with the call.
    attr_accessor :subscriber_id

    # Sequential identifier of the Member associated with the call.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Vba::CallTrackingSubscriber` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Vba::CallTrackingSubscriber`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [call_key, subscriber_id, member_seq].hash
    end
